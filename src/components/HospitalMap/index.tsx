...
const HospitalMap: React.FC<HospitalMapPropsType> = ({
  hospitals,
  viewState,
  setViewState,
}) => {
  ...
  const getPopOverContent = (hospital: IHospital) => {
    return (
      <div className="text-md">
        <div>
          <strong>HealthCare Group Name: </strong>
          {hospital.HealthcareGroupName}
        </div>
        <div>
          <strong>No. of Sites: </strong>
          {hospital.NumOfSites}
        </div>
        <div>
        ...
        <div>
          ...
          {hospital.TrakProduct}
        </div>
        <div
          className="text-blue-600 cursor-pointer hover:underline"
          onClick={() =>
            redirectToGoogleEarth(hospital.Latitude, hospital.Longitude)
          }
        >
          View Google Earth 3D&nbsp;&nbsp;
  ...
  return (
    <div className="w-4/5 h-full rounded-lg overflow-clip shadow-lg">
      ...
      >
        <NavigationControl
          position="top-left"
          style={{ scale: "1.5", transform: "translate(40%, 30%)" }}
        />
        ...
        {hospitals.map((hospital, index) => {
          return (
            ...
            >
              <Popover
                title={<strong className="text-lg">{hospital.SiteName}</strong>}
                content={getPopOverContent(hospital)}
              >
                <div>
                  <svg
